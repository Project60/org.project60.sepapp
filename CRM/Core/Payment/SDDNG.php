...
class CRM_Core_Payment_SDDNG extends CRM_Core_Payment {
  ...
  /**
   * Should the first payment date be configurable when setting up back office
   * recurring payments. In the case of Authorize.net this is an option
   *
   * @return bool
   */
   ...
   *
   * In practice all processors that use the baseIPN function to finish
   * transactions or call the completetransaction api support this by looking
   * up previous contributions in the series and, if there is a prior
   * contribution against a pledge, and the pledge is not complete, adding the
   * new payment to the pledge.
   *
  ...
  public function doDirectPayment(&$params) {
    ...
    if ($bad_iban) {
      CRM_Sepapp_Configuration::log("IBAN issue: {$bad_iban}", CRM_Sepapp_Configuration::LOG_LEVEL_ERROR);
    ...
    if ($bad_bic) {
      CRM_Sepapp_Configuration::log("BIC issue: {$bad_bic}", CRM_Sepapp_Configuration::LOG_LEVEL_ERROR);
    ...
    if (self::$_pending_mandate) {
      if (isset($params['contributionID']) && isset(self::$_pending_mandate['contributionID'])) {
        if ($params['contributionID'] != self::$_pending_mandate['contributionID']) {
          CRM_Sepapp_Configuration::log("Pending mandate found. This is a workflow error.", CRM_Sepapp_Configuration::LOG_LEVEL_ERROR);
          throw new PaymentProcessorException("SDD PaymentProcessor NG: workflow broken.");
        }
      }
    }
   ...
   *   Currently supported:
   *   - contributionPageRecurringHelp (params: is_recur_installments,
   *   is_email_receipt)
   *
   * @param array $params
    ...
    if (version_compare(CRM_Utils_System::version(), '4.6.10', '<')) {
      ...
      return [
        'account_holder' => [
          'htmlType' => 'text',
          'name' => 'account_holder',
          'title' => ts('Account Holder', ['domain' => 'org.project60.sepapp']),
          'cc_field' => TRUE,
          'attributes' => [
            'size' => 20,
            'maxlength' => 34,
        ...
        'bank_name' => [
          'htmlType' => 'text',
          'name' => 'bank_name',
          'title' => ts('Bank Name', ['domain' => 'org.project60.sepapp']),
          'cc_field' => TRUE,
          'attributes' => [
            'size' => 34,
            'maxlength' => 64,
